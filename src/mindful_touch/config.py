...
class NotificationConfig(BaseModel):
    ...
    title: str = Field(default="Mindful Moment", description="Notification title")
    message: str = Field(
        default="Take a gentle pause 🌸", description="Notification message"
    )
    duration_seconds: int = Field(
        default=3, ge=1, le=30, description="How long notification stays visible"
    )
    cooldown_seconds: int = Field(
        default=10, ge=5, le=300, description="Minimum time between notifications"
    )
...
class CameraConfig(BaseModel):
    ...
    device_id: int = Field(
        default=0, ge=0, description="Camera device ID (usually 0 for default camera)"
    )
    width: int = Field(
        default=640, ge=320, le=1920, description="Camera capture width in pixels"
    )
    height: int = Field(
        default=480, ge=240, le=1080, description="Camera capture height in pixels"
    )
    fps: int = Field(
        default=30, ge=10, le=60, description="Frames per second for camera capture"
    )
...
class PrivacyConfig(BaseModel):
    ...
    log_detections: bool = Field(
        default=False, description="Whether to log detection events"
    )
...
class ConfigManager:
    ...
    def __init__(self, app_name: str = "mindful-touch") -> None:
        ...
        self._default_config_file = (
            Path(__file__).parent.parent.parent / "config" / "default_settings.json"
        )
    ...
    def load_config(self) -> AppConfig:
        ...
        # Load default configuration as fallback
            ...
            try:
                with open(self._default_config_file, encoding="utf-8") as f:
                    config_data = json.load(f)
                print(
                    f"📋 Using default configuration from {self._default_config_file}"
                )
            except (OSError, json.JSONDecodeError) as e:
            ...
            adjustments.update(
                {
                    "notifications": {
                        "duration_seconds": 5
                    }  # Linux notifications might need longer duration
                }
            )
        elif system == "windows":
