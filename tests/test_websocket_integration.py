...
class TestEndToEndIntegration:
    ...
    def backend_process(self):
        ...
        # Use mock camera in CI environments or when no camera is available

        use_mock_camera = os.getenv("CI", "").lower() == "true" or os.getenv("GITHUB_ACTIONS", "").lower() == "true"

        cmd = [sys.executable, "-m", "backend.detection.main", "--headless"]
        if use_mock_camera:
            cmd.append("--mock-camera")
            logger.info("Using mock camera for CI environment")
        ...
        for _attempt in range(10):  # Try for 10 seconds
            try:
    ...
    async def test_backend_ping_pong(self, backend_process):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            pong_received = False
            for _attempt in range(10):  # Try up to 10 messages
    ...
    async def test_backend_region_toggles(self, backend_process):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            toggle_message = {"type": "toggle_region", "region": "scalp", "enabled": True}
            await websocket.send(json.dumps(toggle_message))
    ...
    async def test_backend_detection_data_stream(self, backend_process):
        ...
        async with websockets.connect(uri) as websocket:
            ...
            for _attempt in range(20):  # Try for up to 20 seconds
                try:
