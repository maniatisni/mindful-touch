...
class SettingsWidget(QWidget):
    ...
    toggle_camera_feed_requested = Signal(bool)
    ...
    def setup_ui(self):
        ...
        # // NEW: Add a new group for display options.
    ...
    def create_detection_settings(self, layout):
        ...
        self.alert_delay_slider.setRange(0, 50)
    ...
    # // NEW: A new method to create the display settings group box.
    def create_display_settings(self, layout):
        """Create display settings section for camera feed."""
        display_group = QGroupBox("Display Options")
        display_layout = QVBoxLayout(display_group)

        self.toggle_feed_button = QPushButton("Show Camera Feed")
        self.toggle_feed_button.setCheckable(True)  # Makes it a toggle button
        self.toggle_feed_button.clicked.connect(self.on_toggle_feed_clicked)

        display_layout.addWidget(self.toggle_feed_button)
        layout.addWidget(display_group)

    # // NEW: Handler for the new button's click event.
    def on_toggle_feed_clicked(self, checked):
        """Handle camera feed toggle button click."""
        if checked:
            self.toggle_feed_button.setText("Hide Camera Feed")
        else:
            self.toggle_feed_button.setText("Show Camera Feed")
        self.toggle_camera_feed_requested.emit(checked)
