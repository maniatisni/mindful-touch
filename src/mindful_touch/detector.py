...
class DetectionResult:
    ...
    frame: Optional[np.ndarray] = None  # Add frame for live feed
...
class HandFaceDetector:
    ...
    def __init__(self, detection_config: DetectionConfig, camera_config: CameraConfig):
        ...
        # Live feed support
        self._show_live_feed = False
        self._live_feed_callback: Optional[Callable] = None

        print("🎯 Hand-face detector initialized")
    ...
    def _normalize_landmark(
        self, landmark, image_width: int, image_height: int
    ) -> Point3D:
        ...
        for idx in self._face_center_landmarks:
            if idx < len(face_landmarks.landmark):
                point = self._normalize_landmark(
                    face_landmarks.landmark[idx], image_width, image_height
                )
    ...
    def _get_hand_tip_points(
        ...
        for idx in tip_indices:
            if idx < len(hand_landmarks.landmark):
                point = self._normalize_landmark(
                    hand_landmarks.landmark[idx], image_width, image_height
                )
    ...
    def _calculate_min_hand_face_distance(
        self, face_center: Point3D, hand_tip_points: List[Point3D]
    ) -> float:
    ...
    def _draw_annotations(
        ...
        """Draw annotations on frame for live feed.
        ...
        # Draw status text
        status_text = (
            "PULLING DETECTED!" if result.is_hand_near_face else "Monitoring..."
        )
        text_color = (0, 0, 255) if result.is_hand_near_face else (0, 255, 0)
        ...
        # Draw FPS
        fps_text = f"Processing: {1000/result.processing_time_ms:.1f} FPS"
        cv2.putText(
            annotated_frame,
            fps_text,
            (width - 200, height - 20),
    ...
    def detect_frame(
        self, frame: np.ndarray, include_frame: bool = False
    ) -> DetectionResult:
        """Process a single frame for hand-face detection.

        Args:
            frame: Input image frame
            include_frame: Whether to include annotated frame in result
        ...
        if include_frame:
            result.frame = self._draw_annotations(frame, result)
    ...
    def _processing_worker(self):
        ...
        while self.is_running:
            try:
                ...
                # Process frame
                result = self.detect_frame(frame, include_frame=self._show_live_feed)
    ...
    def start_detection(self, show_live_feed: bool = False) -> bool:
        ...
        self._show_live_feed = show_live_feed
        ...
        if show_live_feed:
            print("📺 Live feed window enabled")
        return True
    ...
    def capture_and_detect(self) -> Optional[DetectionResult]:
        """Capture a frame and perform detection asynchronously.

        Returns:
            Detection result or None if no result available
        """
        ...
        # Put frame in processing queue (drop if full)
        try:
    ...
    def get_live_frame(self) -> Optional[np.ndarray]:
        """Get the latest annotated frame for live display.

        Returns:
            Latest annotated frame or None
        """
    ...
    def calibrate(self, duration_seconds: int = 10) -> dict:
            ...
            while time.time() - start_time < duration_seconds:
                ...
                if result:
                    ...
                    if frame_count % 30 == 0:
                        ...
                        print(
                            f"   {elapsed:.1f}s: Face={result.face_detected}, "
                    ...
                    if (
                        ...
                        and result.hands_detected > 0
                        and result.min_hand_face_distance_cm is not None
                    ):
                        calibration_data.append(result.min_hand_face_distance_cm)
                        detection_count += 1

                time.sleep(0.033)  # ~30fps for calibration

        finally:
            self.stop_detection()

        print(
            f"   Processed {frame_count} frames, collected {len(calibration_data)} distance measurements"
        )
        ...
        print(
            f"   Distance range: {stats['min_distance']:.1f}cm - {stats['max_distance']:.1f}cm"
        )
    ...
    def update_config(
        ...
        if detection_config:
            self.detection_config = detection_config
            print(
                f"🔄 Detection config updated - threshold: {detection_config.hand_face_threshold_cm}cm"
            )
        ...
        if camera_config:
            ...
            print(
                f"🔄 Camera config updated - {camera_config.width}x{camera_config.height}"
            )


def create_detector(
    detection_config: DetectionConfig, camera_config: CameraConfig
) -> HandFaceDetector:
